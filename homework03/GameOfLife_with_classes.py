...
class GameOfLife:
    ...
    def cell_list(self, randomize=True):
        ...
        if randomize:
            ...
            self.clist = [[0 for _ in range(self.cell_width)]
                          for _ in range(self.cell_height)]
        return self.clist
    ...
    def get_neighbours(self, cell):
        ...
        for neighbour in positions:
            if (0 <= neighbour[0] + row < self.cell_height) and\
               (0 <= neighbour[1] + col < self.cell_width):
    ...
    def update_cell_list(self, cell_list):
        ...
        """
        new_clist = [[0 for _ in range(self.cell_width)]
                     for _ in range(self.height)]
        for row in range(self.cell_height):
            for col in range(self.cell_width):
                if cell_list[row][col]:
                    if 1 < self.get_neighbours((row, col)).count(1) < 4:
                        new_clist[row][col] = 1
                    else:
                        new_clist[row][col] = 0
                else:
